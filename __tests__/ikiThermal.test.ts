...
Kvito Nr. 168/645/104148 Kasa 0027`;
    ...
    test('Kvito Nr. band is FULL-WIDTH and BENT, anchored in Y to the receipt-no value', () => {
        ...
        // FULL WIDTH: spans the receipt left→right (x0 … 740), not just a word span
        expect(rn.xLeft).toBeLessThanOrEqual(50);
        expect(rn.xRight).toBeGreaterThanOrEqual(740);
        // BENT: the two sides have different Y (follows the receipt tilt)
        expect(rn.yRightTop).not.toBeCloseTo(rn.yLeftTop!, 1);
        // sits on the value row (~300–342), not drifted off it
        expect(rn.yLeftTop!).toBeLessThan(330);
        expect(rn.yLeftBottom!).toBeGreaterThan(310);
    });
...
describe('IKI column engine — row reconstruction from words', () => {
    ...
    test('receipt-60: Kvito Nr band uses the full line bbox when the "Kvito" word was dropped by OCR', () => {
        // The "Kvito Nr. 168/645/104148 Kasa 0027" line: MLKit kept only a stray
        // "26, 52" amount (physically on the right, but first in reading order). The
        // band must cover the whole line, not anchor to that stray amount.
        const lines: IkiLine[] = [
            wl('PVM mokėtojo kodas LT101937219', 0, [word('PVM', 0, 60, 0), word('kodas', 205, 300, 0), word('LT101937219', 310, 500, 0)]),
        ...
        const rn = footer.lineRegions!.find((r) => r.kind === 'receiptNo')!;
        expect(rn).toBeTruthy();
        // covers the whole line (xLeft ~71, not anchored to the stray amount at x778)
        expect(rn.xLeft).toBeLessThan(200);
        expect(rn.xRight).toBeGreaterThan(820);
    });

    test('receipt-61: total + Laikas footer bands stretch the full line when value words are dropped', () => {
        ...
        expect(total.xLeft).toBeLessThan(100);    // starts at "SUMA"
        expect(total.xRight).toBeGreaterThan(800); // … and stretches across the 26,52 EUR
        const dt = footer.lineRegions!.find((r) => r.kind === 'dateTime')!;
        expect(dt).toBeTruthy();
        expect(dt.xRight).toBeGreaterThan(800);    // covers Laikas 21:07:53 on the right
    });
    ...
    test('receipt-64: the company-code band is FULL-WIDTH even when OCR keeps only "PVN"', () => {
        const lines: IkiLine[] = [
        ...
        // spans (near) the full receipt width — not just the surviving 60px "PVN" word
        expect(code.xRight - code.xLeft).toBeGreaterThan(400);
        expect(code.xLeft).toBeLessThanOrEqual(60);
    });
