...
/**
 * Image cleanup admin tab.
 *
 * Flow:
 *   1. On mount: GET /queue → if empty, POST /claim-batch.
 *   2. Render the top card. User taps a candidate thumbnail → highlight.
 *   3. "Patvirtinti" → adopt; advance to next card.
 *   4. "Įkelti savo" → expo-image-picker → upload via presigned URL →
 *      adopt as admin upload.
 *   5. "Pašalinti" → remove (mainly for flagged "wrong image" cases).
 *   6. "Praleisti" → skip. Lease completes; card disappears.
 *   7. When local stack hits 0 → auto claim-batch again.
 *   8. Inline "Atstatyti" banner when lastPropagation present.
 */
...
export default function AdminImagesScreen() {
    ...
    // Reset selection when the top card changes.
    ...
    const onConfirmAdopt = useCallback(() => {
        ...
        // Admin uploads stage themselves as a synthetic 'pending_upload'
        // candidate flagged with __adminUpload. Send the correct
        // sourceType so the server logs it under admin_upload (bypassing
        // the pending queue) rather than user_upload_approved.
    ...
    const onRejectPending = useCallback((cand: AdminImageCandidate) => {
        ...
        // Don't advance — rejecting a single candidate doesn't resolve
        // the card. Refresh the current card so it shows without that
        // candidate.
        (async () => {
            if (actioning) return;
            setActioning(true);
            try {
                ...
                // Mutate the local card to remove the rejected candidate
                // without refetching the whole batch.
    ...
    const onUploadOwn = useCallback(async () => {
        ...
        // Same compression pipeline as the receipt create-product path —
        // resize to 900px then re-compress if the result is still too
        // heavy. Keeps catalog images small without burning bandwidth on
        // 5 MB phone-camera JPEGs.
        setUploading(true);
        try {
            ...
            // Get presigned URL + PUT the binary.
            ...
            // Stage the image as a NEW candidate on the current card
            // and auto-select it. Admin still has to tap "Patvirtinti"
            // to commit — this way they see what they just uploaded
            // before it goes public, and a wrong photo can be cancelled
            // by tapping a different candidate or Praleisti instead.
            setRows(prev => prev.map((r, i) => {
                if (i !== 0) return r;
                const adminCand = {
                    imageUrl: filePath,
                    ...
                    sourceSpId: null,
                    // No pendingUploadId — this is an admin upload that
                    // bypasses the pending queue. The adopt path uses
                    // sourceType='admin_upload' when we commit, so we
                    // map this synthetic candidate to that source.
                };
                // Mark this synthetic entry so the adopt handler knows
                // to send admin_upload (not pending_upload) to the server.
    ...
    const onRevert = useCallback(() => {
        ...
        // Note: lastPropagation.id is the ImagePropagationLog id, not the
        // audit id. To revert through the server's audit-based endpoint
        // we need the audit row instead. For v1 the inline button is
        // informational; the dedicated audit-log screen is where the
        // user does full reverts. Leaving this as a placeholder action
        // that pops a hint, so the data row stays visible.
    ...
    const totalInBatch = rows.length;
    const done = BATCH_SIZE - totalInBatch;
            ...
            <ScrollView contentContainerStyle={styles.cardScroll}>
                <View style={styles.card}>
                    ...
                    <View style={[
                        ...
                        {selectedImageUrl ? (
                            // Selected candidate (cross-chain, link, pending,
                            // or admin's own upload). Replaces the SP's
                            // current image in the preview so the admin sees
                            // exactly what Patvirtinti will publish.
                            <Image
                    ...
                    >
                        {/* First slot is always "Įkelti" — admin's own upload
                            entry point, sitting in the same horizontal strip
                            so all image sources are visible together. The
                            other footer buttons (Upload / Remove) move out;
                            footer now has only Patvirtinti. */}
...
const makeStyles = (c: AppTheme) => StyleSheet.create({
    ...
    noCandidates: { fontSize: 13, color: c.textMuted, textAlign: 'center', paddingVertical: 16 },
    ...
    footer: {
        ...
        borderTopWidth: 1, borderTopColor: c.borderSubtle, gap: 8,
    },
    primaryBtn: {
        backgroundColor: c.primary, paddingVertical: 14, borderRadius: 12, alignItems: 'center',
    },
