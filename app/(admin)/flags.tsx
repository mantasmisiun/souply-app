...
/**
 * Unified Flags admin tab.
 *
 * Each card represents one `(receiptId, lineIdx)` flagged by 1..N users.
 * The card carries every editable field needed to resolve any flag type
 * (name/brand/amount/unit/isWeighable/image/price/discount) — sub-sections
 * the user actually flagged are accented in primary (pink) so the admin
 * sees which fields need attention.
 *
 * Image is intentionally read-only here. The dedicated Images tab owns
 * the cross-chain candidate flow + pending uploads; a flag on the image
 * is "go check the Images tab" rather than an inline picker.
 *
 * Confirm submits a partial payload — only fields the admin actually
 * touched go in `sp`. Price/discount suspect flags log audit rows + flip
 * `Price.priceVerified=0` so the receipt comparison stops trusting that
 * row until the parser is fixed.
 */
...
/**
 * Read a Blob into a `data:` URI. RN's `Image` source accepts data
 * URIs directly, which sidesteps every quirk of passing custom
 * headers through native image loaders (iOS RN drops them on some
 * paths, especially when the underlying URL is on a non-https LAN
 * host). FileReader is what `@react-native-async-storage` and most
 * RN blob libs use under the hood.
 */
...
export default function AdminFlagsScreen() {
    ...
    // Form state — re-seeded from the top card each time `currentCard.flagKey` changes.
    // "Atpažintas tekstas" — the OCR'd label on the receipt (SP.storeProductName).
    const [ocrInput, setOcrInput] = useState('');
    // "Pavadinimas" typeahead — what Product the SP points at.
    ...
    // "Kategorija" typeahead — only existing categories, no create path.
    ...
    // Kaina kvite — direct edit.
    const [priceEditing, setPriceEditing] = useState(false);
    const [priceInput, setPriceInput] = useState('');
    // Nuolaida kvite — three states:
    //   • 'keep'     — admin hasn't touched it; do nothing on submit
    //   • 'editing'  — input visible with current/typed value
    //   • 'removing' — admin tapped trash on an existing discount; on
    //                  submit send `promoPrice: null`
    type DiscountMode = 'keep' | 'editing' | 'removing';
    const [discountMode, setDiscountMode] = useState<DiscountMode>('keep');
    const [discountInput, setDiscountInput] = useState('');

    // Image picker state. `selectedImageUrl` defaults to the SP's
    // current image URL so the current slot starts pink-bordered. Tap
    // a different candidate or upload your own → that becomes the
    // selection. `extraCandidates` carries admin uploads staged this
    // session (not yet committed to the server's PendingImageUpload
    // table — confirmed when the admin taps Patvirtinti).
    const [selectedImageUrl, setSelectedImageUrl] = useState<string | null>(null);
    const [extraCandidates, setExtraCandidates] = useState<AdminImageCandidate[]>([]);
    const [uploading, setUploading] = useState(false);

    // The receipt-crop endpoint needs `X-Admin-Id` for auth. Rather
    // than rely on `<Image source={{uri, headers}}>` (drops headers on
    // some iOS code paths, especially over LAN HTTP), fetch the JPEG
    // bytes via `fetchFlaggedReceiptCrop`, encode as a `data:` URI,
    // and feed that directly to `<Image>`. Same pattern is used in
    // the receipt-detail screen for authed images.
    const [crop, setCrop] = useState<CropState>({ status: 'loading' });
    // Computed from the crop image's intrinsic dimensions once it
    // loads, so the box hugs the actual crop instead of reserving a
    // tall placeholder. Null → use a slim default for the loading /
    ...
    useEffect(() => {
        ...
        // Re-seed all card-local state when the top card changes.
        ...
        setPriceInput(currentCard.receiptPrice.price !== null
            ? String(currentCard.receiptPrice.price) : '');
        setDiscountMode('keep');
        setDiscountInput(currentCard.receiptPrice.promoPrice !== null
            ? String(currentCard.receiptPrice.promoPrice) : '');

        // Re-seed image picker state. Default selection is whatever
        // the SP currently has (so the current image starts with the
        // pink "selected" outline). Drop any uploads staged for the
        // previous card.
        ...
        // Fetch the crop with auth, encode as a data URI, hand off to
        // <Image>. Cancellable so a fast tap-tap-tap through cards
        // doesn't paint a stale crop after the user moved on.
        setCrop({ status: 'loading' });
        let cancelled = false;
        (async () => {
            try {
                const res = await fetchFlaggedReceiptCrop(
                    currentCard.receiptId,
                    currentCard.lineIdx,
                );
                if (cancelled) return;
                if (!res) {
                    // 404 / 422 — no region data or multi-page receipt.
                    setCrop({ status: 'missing' });
                    return;
                }
                ...
                setCrop({
                    status: 'error',
                    message: String(e?.message ?? 'fetch failed'),
                });
            }
        })();
    ...
    // Debounced product typeahead. Fires only when the user is
    // interacting with the field (productSearchOpen), and only when
    // the input differs from a freshly-picked product's name (so the
    // dropdown doesn't reappear right after the admin taps a result).
    useEffect(() => {
        if (!productSearchOpen) return;
        const picked = productSuggestions.find(s => s.id === productPickedId)
            ?? (currentCard && productPickedId === currentCard.productId
                ? { name: currentCard.productName } : null);
        if (picked && productInput.trim() === picked.name.trim()) {
            setProductSuggestions([]);
            return;
        }
        ...
        const timer = setTimeout(async () => {
            try {
                const rows = await searchAdminProducts(q, 8);
    ...
    useEffect(() => {
        ...
        const picked = categorySuggestions.find(s => s.id === categoryPickedId)
            ?? (currentCard && categoryPickedId === currentCard.categoryId
                ? { name: currentCard.categoryName } : null);
        ...
        const timer = setTimeout(async () => {
            try {
                const rows = await searchAdminCategories(q);
    ...
    const advance = useCallback(() => {
        // Pop the top card. Same explicit-claim pattern as the amounts
        // tab — empty-state shows "Imti naują paketą", admin taps to
        // claim the next batch.
        setRows(prev => prev.slice(1));
    }, []);

    // Admin-uploaded image flow: pick from library → compress → PUT
    // to presigned MinIO URL → stage as a synthetic candidate marked
    // __adminUpload. The actual SP write happens at Patvirtinti time
    // via `adoptImageCandidate(sourceType='admin_upload')`. Same shape
    // as the Images tab's `onUploadOwn`.
    const onUploadOwn = useCallback(async () => {
        ...
        try {
            ...
            const uploadUrlRes = await fetch(`${API_BASE_URL}/api/store-products/upload-url`, {
                ...
                body: JSON.stringify({
                    filename: `admin-flag-${currentCard.spId}-${Date.now()}.jpg`,
                    mimeType: 'image/jpeg',
                }),
            });
            ...
            const adminCand: AdminImageCandidate = {
                imageUrl: filePath,
                sourceType: 'pending_upload',
                sourceSpId: null,
            };
            // Marker so the adopt call sends sourceType='admin_upload'
            // (bypasses the PendingImageUpload approval workflow).
    ...
    const buildConfirmPayload = (card: AdminFlagQueueRow): ConfirmFlagPayload => {
        ...
        // "Atpažintas tekstas" — SP.storeProductName (chain-specific OCR label).
        const trimmedOcr = ocrInput.trim();
        const currentOcr = (card.sp.storeProductName ?? '').trim();
        if (trimmedOcr.length > 0 && trimmedOcr !== currentOcr) {
            sp.storeProductName = trimmedOcr;
        }

        const amountNum = parseFloat(amountInput.replace(',', '.'));
        if (Number.isFinite(amountNum) && amountNum > 0 && amountNum !== card.sp.amount) {
            sp.amount = amountNum;
        }
        if (unitInput !== card.sp.unit) sp.unit = unitInput;
        const canToggleWeighable = unitInput === 'kg' && amountInput.replace(',', '.').trim() === '1';
        if (canToggleWeighable && isWeighable !== card.sp.isWeighable) {
            sp.isWeighable = isWeighable;
        }

        const payload: ConfirmFlagPayload = {};
        if (Object.keys(sp).length > 0) payload.sp = sp;

        // Product re-link: pick existing OR create new.
        const trimmedProduct = productInput.trim();
        if (trimmedProduct.length > 0 && trimmedProduct !== card.productName.trim()) {
            // Admin actually changed the field. Two outcomes:
            //  - productPickedId set + matches a known suggestion or
            //    the current product → pick mode (re-link to that id).
            //  - Otherwise treat as create — admin typed a name with
            //    no exact match in the catalog.
            ...
            // Field text unchanged but admin picked a same-named
            // sibling product from the dropdown (rare; still honour).
            payload.productLink = { mode: 'pick', productId: productPickedId };
        }

        // Category change (applied to the effective product post-link).
        if (categoryPickedId !== null && categoryPickedId !== card.categoryId) {
            payload.categoryId = categoryPickedId;
        }

        const receiptPrice: ConfirmFlagPayload['receiptPrice'] = {};
        if (priceEditing) {
            const n = parseFloat(priceInput.replace(',', '.'));
            if (Number.isFinite(n) && n >= 0 && n !== card.receiptPrice.price) {
                receiptPrice.price = n;
            }
        }
        if (discountMode === 'removing') {
            ...
            if (Number.isFinite(n) && n >= 0 && n !== card.receiptPrice.promoPrice) {
                receiptPrice.promoPrice = n;
            }
        }
    ...
    const onConfirm = useCallback(async () => {
        ...
        // If the admin picked a different image (current → some
        // candidate, or current → uploaded), commit the image change
        // through the same `adopt-candidate` endpoint the Images tab
        // uses. That writes the ImagePropagationLog + handles pending
        // upload approval, keeping the audit trail consistent across
        // tabs. Image is *not* included in `payload.sp.imageUrl` so
        // the confirm controller doesn't double-write the same value.
        const allCandidates: AdminImageCandidate[] = [
            ...extraCandidates,
            ...currentCard.imageCandidates,
        ];
        const currentImg = currentCard.sp.imageUrl ?? null;
        const imageChanged = selectedImageUrl !== currentImg;
        const chosen = imageChanged
            ? allCandidates.find(c => c.imageUrl === selectedImageUrl) ?? null
            : null;
        ...
        try {
            ...
            Alert.alert(msg.includes('429')
                ? t('admin.flags.rateLimitToast')
                : t('admin.flags.errorToast'));
        } finally {
            setActioning(false);
        }
        ...
        <View style={styles.page}>
            ...
            <ScrollView contentContainerStyle={styles.cardScroll}>
                <View style={styles.card}>
                    ...
                    {/* Receipt crop preview. Fetched with auth, encoded
                        as a data URI to bypass iOS RN's header quirks. */}
                    ...
                    {crop.status === 'ready' ? (
                        ...
                        <View style={[styles.cropFallback, styles.cropFallbackPlaceholder]}>
                            <Text style={styles.cropFallbackText}>
                                {crop.status === 'missing'
                                    ? t('admin.flags.receiptCropMissing')
                                    : crop.message}
                            </Text>
                        </View>
                    )}

                    {/* Atpažintas tekstas — the chain-specific OCR'd
                        label (SP.storeProductName). Pure admin info,
                        not user-flagged (the user never sees this). */}
                    <Section
                        styles={styles}
                        title={t('admin.flags.sectionOcr')}
                        flagged={false}
                        flaggedLabel=""
                    >
                        <TextInput
                    ...
                    {/* Pavadinimas — what the user saw + flagged. The
                        field doubles as a Product search. Edits go to
                        SP.productId (pick) or create a new Product. */}
                    <Section
                        styles={styles}
                        title={t('admin.flags.sectionName')}
                        flagged={flagged.name}
                        flaggedLabel={t('admin.flags.flaggedPill')}
                    >
                        <View style={styles.typeaheadRow}>
                            <TextInput
                                ...
                                onChangeText={(text) => {
                                    ...
                                    // Typing re-opens the dropdown so
                                    // the lock-in button can hide it
                                    // while still letting the admin
                                    // edit back into typeahead mode.
                                ...
                                {productSuggestions.map((s) => (
                                    <TouchableOpacity
                                        ...
                                        onPress={() => {
                                            ...
                                            setProductSearchOpen(false);
                                            setProductSuggestions([]);
                                            // Picking a Product brings
                                            // its category along — pre-
                                            // select that in Kategorija
                                            // so it lines up.
                        ...
                        {/* Hint about what'll happen on Patvirtinti. */}
                        {(() => {
                            const trimmed = productInput.trim();
                            const unchanged =
                                trimmed === currentCard.productName.trim() &&
                                productPickedId === currentCard.productId;
                            if (unchanged) return null;
                            if (productPickedId !== null) {
                                return <Text style={styles.actionHint}>{t('admin.flags.willRelink')}</Text>;
                            }
                            if (trimmed.length > 0) {
                                return <Text style={styles.actionHint}>{t('admin.flags.willCreate', { name: trimmed })}</Text>;
                            }
                            return null;
                        })()}
                    </Section>

                    {/* Kategorija — typeahead over existing leaf
                        categories. No create-new path. */}
                    <Section
                        styles={styles}
                        title={t('admin.flags.sectionCategory')}
                        flagged={false}
                        flaggedLabel=""
                    >
                        <View style={styles.typeaheadRow}>
                    ...
                    <Section
                        styles={styles}
                        title={t('admin.flags.sectionAmount')}
                    ...
                    {/* Image picker — horizontal strip: upload + current
                        (pink-bordered as the default selection) + the
                        rest of the available candidates. Same shape as
                        the Images tab so the admin can fix a flagged
                        image inline without bouncing tabs. */}
                    <Section
                        styles={styles}
                        title={t('admin.flags.sectionImage')}
                        flagged={flagged.image}
                        flaggedLabel={t('admin.flags.flaggedPill')}
                    >
                        <ScrollView
                            horizontal
                        ...
                        >
                            <View style={styles.candidateWrap}>
                                ...
                                <Text style={styles.candidateLabel} numberOfLines={2}>
                                    {uploading
                                        ? t('admin.flags.imageUploading')
                                        : t('admin.flags.imageUpload')}
                                </Text>
                            </View>

                            {/* Current image slot — always second, pink-
                                bordered when selected (the default). Tap
                                to re-select if the admin scrolled through
                                other candidates and wants to revert. */}
                            <View style={styles.candidateWrap}>
                                <Pressable
                                    onPress={() => setSelectedImageUrl(currentCard.sp.imageUrl ?? null)}
                                >
                                    {currentCard.sp.imageUrl ? (
                                        <Image
                                            ...
                                            style={[
                                                styles.candidateImage,
                                                selectedImageUrl === currentCard.sp.imageUrl &&
                                                    styles.candidateImageSelected,
                                            ]}
                                            resizeMode="cover"
                                        />
                                    ) : (
                                        <View style={[
                                            ...
                                            selectedImageUrl === null && styles.candidateImageSelected,
                                        ]}>
                    ...
                    <Section
                        styles={styles}
                        title={t('admin.flags.sectionPrice')}
                        flagged={flagged.price}
                        flaggedLabel={t('admin.flags.flaggedPill')}
                    >
                        {priceEditing ? (
                            ...
                            <View style={styles.priceRow}>
                                ...
                                <View style={styles.priceEditWrap}>
                                    <Text style={styles.priceValue}>
                                        {currentCard.receiptPrice.price !== null
                                            ? `€${currentCard.receiptPrice.price.toFixed(2)}`
                                            : '—'}
                                    </Text>
                                    <TouchableOpacity
                                        style={styles.priceIconBtn}
                    ...
                    <Section
                        styles={styles}
                        title={t('admin.flags.sectionDiscount')}
                        flagged={flagged.discount}
                        flaggedLabel={t('admin.flags.flaggedPill')}
                    >
                        {(() => {
                            const original = currentCard.receiptPrice.promoPrice;
                            // Removing state: render a strikethrough +
                            // an undo affordance.
                            if (discountMode === 'removing') {
                                return (
                                    <View style={styles.priceRow}>
                            ...
                            // Editing or adding: render an input.
                                    ...
                                    <View style={styles.priceRow}>
                                        ...
                                        <View style={styles.priceEditWrap}>
                                            ...
                                            {original !== null && (
                                                <TouchableOpacity
                                                    style={styles.priceIconBtn}
                                                    onPress={() => setDiscountMode('removing')}
                                                    hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}
                                                >
                            ...
                            // Default 'keep' state. If OCR detected a
                            // promo, show value + pencil + trash. If
                            // not, show an "Pridėti nuolaidą" button.
                            if (original !== null) {
                                return (
                                    <View style={styles.priceRow}>
                                        <Text style={styles.priceLabel}>{t('admin.flags.promoLabel')}</Text>
                                        <View style={styles.priceEditWrap}>
                                            ...
                                            <TouchableOpacity
                                                style={styles.priceIconBtn}
                                                onPress={() => setDiscountMode('editing')}
                                                hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}
                                            >
                                                <Ionicons name="pencil" size={16} color={colors.textSecondary} />
                                            </TouchableOpacity>
            ...
            <View style={styles.sectionHeader}>
                ...
                {flagged && (
                    <View style={styles.flaggedPill}>
                        <Text style={styles.flaggedPillText}>{flaggedLabel}</Text>
                    </View>
                )}
            </View>
            {children}
